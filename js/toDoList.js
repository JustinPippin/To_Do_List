...
function createListElement() {
    ...
    //Add it to our ul
    ul.appendChild(li);
    //Clear our text input after we create our new list item
    input.value = "";

    //Function to handle showing a list item as complete
    function markDone() {
        //If the li does not have a class "done" applied to it, it will be applied. If it doesn, it will be taken off
...
//Makes it so that when the submit button is clicked, the createListItemSubmitButton function is called
submitButton.addEventListener("click", createListItemSubmitButton);

//This event listener will detect when keys are pressed while input is active
